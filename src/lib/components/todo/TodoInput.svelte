<script lang="ts">
	import { deleteTodo, editTodo, toggleComplete } from "$lib/store/TodoStore";
	import type { ITodo } from "../../../types";

  function handleInput(e:Event) {
    editTodo(todo.id, (<HTMLInputElement>e.target).value)
  }
...
<div class="flex items-center justify-between rounded-md border-2 border-gray-one px-5 py-4">
  <div class="flex w-full max-w-lg items-center justify-start">
    ...
    <input 
      class="h-4 w-4 rounded border-gray-three bg-cream-four text-green-four focus:border-green-five focus:outline focus:outline-2 focus:outline-offset-2 focus:outline-green-five"
      id={`${todo.id}-checkbox`} 
      type="checkbox" 
    ...
    <input 
    class="w-full ml-5 flex-1 text-ellipsis rounded-none border-x-0 border-t-0 border-b border-dashed border-b-gray-two px-0 pb-1 text-base font-normal text-gray-three placeholder:text-gray-two focus:border-gray-three focus:outline-none focus:ring-0"
      id={`${todo.id}-text`} 
      type="text" 
